...
<strong>Before uploading, ensure your document includes:</strong>
...
<p><b id="revisedPaperFileName"></b></p>
<p class="small text-muted">Size: <span id="revisedPaperFileSize"></span></p>
<button type="button" class="btn btn-sm btn-outline-danger" onclick="removeRevisedPaper()">Remove</button>
</div>
...
const revisedPaperInput=$("#revised_fullpaper");

function checkSubmit(){

const hasRevised=revisedPaperInput.length && revisedPaperInput[0].files.length>0;

let hasPresentationFile=false;

if(pptInput.length && pptInput[0].files.length>0){
hasPresentationFile=true;
}
...
submitBtn.prop("disabled",!(hasRevised && hasPresentationFile));

}

// Revised Full Paper
$("#revisedPaperUploadArea").on("click",()=>revisedPaperInput.click());

revisedPaperInput.on("change",function(){
...
// Supporting Docs
$("#docsUploadArea").on("click",()=>$("#supporting_docs").click());

$("#supporting_docs").on("change",function(){
...
html+=`<li>${files[i].name} (${(files[i].size/1048576).toFixed(2)} MB)</li>`;
}

html+='</ul>';

$("#docsFileList").html(html);
